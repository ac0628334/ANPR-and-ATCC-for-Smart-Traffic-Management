    ...
    if request.method == 'POST':
        ...
        try:
            with connection.cursor() as cursor:
                ...
                check_query = "SELECT id FROM login_details WHERE username = %s"
                ...
                insert_query = "INSERT INTO login_details (username, password) VALUES (%s, %s)"
...
def upload_video():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            #  ADD THIS BLOCK (CREATES static/uploads IF MISSING)
            if not os.path.exists(app.config['UPLOAD_FOLDER']):
                os.makedirs(app.config['UPLOAD_FOLDER'])
                
            # Save the file in the static/uploads directory
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            return render_template('upload_video.html', success=True, message="Uploaded successfully!", filename=filename)
         
        else:
            return render_template('upload_video.html', message="Video not uploaded!",error="Invalid file type. Allowed: mp4, avi, mov, mkv.")
...
def accident_detection():
    ...
    try:
        ...
        model_path = r"D:\ANPR-and-ATCC-for-Smart-Traffic-Management-main\accident_best.pt"  # Use your YOLO model path
        detector = AccidentDetection(model_path, conf_threshold=0.4, enable_gui=False)
