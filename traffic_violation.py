...
# manually assining the path of the tesseract if u have set the path then in envirnment variable u can skip this step 
...
license_plate_cascade = cv2.CascadeClassifier(r"D:\ANPR-and-ATCC-for-Smart-Traffic-Management\models\YOLO\haarcascade_russian_plate_number.xml")
...
def update_database_with_violation(plate_number, host, user, password, database): # table -YB0880 -1 
    try:
        connection = mysql.connector.connect(
            host = host,
            user = user,
...
def print_all_violations(host, user, password, database):
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute("SELECT plate_number, violation_count FROM license_plates ORDER BY violation_count DESC") #YB -2, TB -1
...
if __name__ == "__main__":
    main(r"D:\ANPR-and-ATCC-for-Smart-Traffic-Management-main\sample detection videos\trafic_vilation demo .mp4")