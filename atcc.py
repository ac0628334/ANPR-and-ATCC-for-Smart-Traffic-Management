...
def load_videos_from_folder(video_input):
    """Load video paths from file, folder, or list."""
    videos = []

    # CASE 1: Single file path (string)
        ...
        if os.path.isfile(video_input) and video_input.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
            ...
            for filename in os.listdir(video_input):
                if filename.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
                    videos.append({
                        "path": os.path.join(video_input, filename),
                        "road_name": filename
                    })
            return videos

    # CASE 2: Flask uploads (list of file paths)
    if isinstance(video_input, list):
        for file in video_input:
            if isinstance(file, str) and os.path.isfile(file):
                videos.append({
                    "path": file,
                    "road_name": os.path.basename(file)
                })
        return videos

    print(f"[ERROR] Invalid video_input: {video_input}")
    return videos
...
if __name__ == "__main__":
    model = YOLO(r"D:\ANPR-and-ATCC-for-Smart-Traffic-Management-main\models\atcc.pt")
    # Single video:
    video_input = r"D:\ANPR-and-ATCC-for-Smart-Traffic-Management-main\sample detection videos\atcc sample.mp4"
    #Folder of videos:
   
   # video_input =r"D:\ANPR-and-ATCC-for-Smart-Traffic-Management-main\input_video\archive"
